...
#define PITCH_LIMIT 0.77f // A little less than PI/4
...
void Camera::processInput(const input::KeyboardInput& kb, const input::MouseInput& mouse, float deltaT) {
	...
	if (kb.isKeyDown(input::KeyboardInput::KC_W)) {
		// Move toward where the camera is looking "at"
		translationOffset += (deltaT * TRANSLATION_UNITS_PER_SECOND) * transform.at();
		doTranslate = true;
	}

	if (kb.isKeyDown(input::KeyboardInput::KC_S)) {
		// Move away from where the camera is looking "at"
		translationOffset -= (deltaT * TRANSLATION_UNITS_PER_SECOND) * transform.at();
		doTranslate = true;
	}
